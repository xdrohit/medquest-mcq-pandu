  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end">
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-2xl h-screen bg-white dark:bg-slate-900 border-l border-slate-300 dark:border-slate-700 overflow-y-auto flex flex-col"
      >
        {/* Header */}
        <div className="sticky top-0 bg-white dark:bg-slate-900 z-10 flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-slate-800">
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{isEdit ? "Edit Exam" : "Create New Exam"}</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">All changes sync live to the database.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 p-6 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {Object.entries(statusConfig).map(([key, cfg]) => (
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-semibold transition-all ${form.status === key ? cfg.color + " scale-[1.02]" : "border-slate-300 dark:border-slate-700 text-slate-500 hover:border-slate-500"}`}
                >
                  {cfg.icon} {cfg.label}
                </button>
              ))}
          ...
          {/* Marks & Timer Row */}
          <div>
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider block mb-3 flex items-center gap-2"><Clock className="w-3.5 h-3.5 text-primary-400" /> Time & Marks Configuration</label>
          ...
          {/* Options Row */}
          <div>
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider block mb-3 flex items-center gap-2"><Target className="w-3.5 h-3.5 text-accent-400" /> Exam Options</label>
            <div className="space-y-3">
              {/* Random Question Order */}
              <label className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-300 dark:border-slate-700 cursor-pointer group hover:border-slate-600 transition-colors">
                <div className="flex items-center gap-3">
                  <Shuffle className="w-4 h-4 text-purple-400" />
                  <div>
                    <p className="text-sm font-semibold text-slate-900 dark:text-white">Random Question Order</p>
    ...
    <div className="flex-1">
      ...
      <div className="px-8 py-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: "Total Exams", value: statCounts.total, color: "text-slate-900 dark:text-white", bg: "bg-slate-50 dark:bg-slate-800 border-slate-300 dark:border-slate-700" },
            { label: "Published", value: statCounts.published, color: "text-green-400", bg: "bg-green-500/10 border-green-500/20" },
            { label: "Drafts", value: statCounts.draft, color: "text-slate-500 dark:text-slate-400", bg: "bg-slate-700/30 border-slate-300 dark:border-slate-700" },
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <div className="relative flex-grow max-w-sm">
            ...
            <input type="text" placeholder="Search exams..." value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
          </div>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => <div key={i} className="h-48 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((exam, idx) => {
              ...
              return (
                <motion.div key={exam._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 flex flex-col gap-4 hover:border-slate-300 dark:border-slate-700 transition-all group relative overflow-hidden">
                    {/* Status stripe */}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-grow min-w-0">
                        <h3 className="text-slate-900 dark:text-white font-bold text-base leading-tight truncate">{exam.title}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-slate-50 dark:bg-slate-800 rounded-lg px-3 py-2">
                        ...
                        <p className="text-slate-900 dark:text-white font-bold">{exam.totalMarks}</p>
                      </div>
                      <div className="bg-slate-50 dark:bg-slate-800 rounded-lg px-3 py-2">
                    ...
                    <div className="flex items-center gap-2 pt-1 border-t border-slate-200 dark:border-slate-800">
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-bold transition-colors ${exam.status === "published" ? "bg-green-500/20 text-green-400 hover:bg-green-500/30" : "bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:text-white"}`}
                      >
                        {exam.status === "published" ? <><EyeOff className="w-3.5 h-3.5" /> Unpublish</> : <><Eye className="w-3.5 h-3.5" /> Publish</>}
                      </button>
                      <button onClick={() => openEdit(exam)} className="p-2 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-primary-400 transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <Link href="/admin/questions" className="p-2 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-accent-400 transition-colors">
                        <BookOpen className="w-4 h-4" />
                      </Link>
                      <button onClick={() => handleDelete(exam._id)} className="p-2 rounded-xl hover:bg-red-500/20 text-slate-500 dark:text-slate-400 hover:text-red-400 transition-colors">
