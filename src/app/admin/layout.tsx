...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    <>
      <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-slate-900 dark:text-white font-bold text-lg leading-none">Daily Dose MCQ</span>
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} onClick={() => setMobileOpen(false)}>
              <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all cursor-pointer group ${isActive ? "bg-primary-50 dark:bg-primary-500/15 text-primary-600 dark:text-primary-400 border border-primary-100 dark:border-primary-500/20" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800"}`}>
                <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? "text-primary-600 dark:text-primary-400" : "text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-300"}`} />
                {label}
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-30 flex items-center justify-between px-4 transition-colors">
        <div className="flex items-center gap-2">
          <Activity className="w-6 h-6 text-primary-500 dark:text-primary-400" />
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex-col fixed left-0 top-0 z-40 transition-colors">
        {SidebarContent}
      </aside>
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="md:hidden fixed top-0 left-0 bottom-0 w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 z-50 flex flex-col"
            >
